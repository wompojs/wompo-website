/*!
 * Wompo v1.1.2 (https://wompo.dev/)
 * Copyright 2024 The Wompo Authors
 * Licensed under MIT (https://github.com/wompojs/wompo/blob/main/LICENSE)
 */
...
const __shouldUpdate = (currentValue, oldValue, dependency) => {
  ...
  const isDynamicNodeToUpdate = currentValue === oldValue && dependency.isTag && dependency.node.nodeName === DYNAMIC_TAG_MARKER.toUpperCase();
...
const _$wompo = (Component, options) => {
  ...
  const WompoComponent = class extends HTMLElement {
    ...
    adoptedCallback() {
      ...
      if (!adoptedStyles[componentName])
        adoptedStyles[componentName] = [];
      if (options.shadow) {
        ...
        if (!this.__updating) {
          mutationRecords.forEach((record) => {
            if (!mutationAttributesExclusions.includes(record.attributeName)) {
              ...
              if (this.props[propName] != newAttrVal)
...
export const useLayoutEffect = (callback, dependencies = null) => {
  ...
  if (!component.hooks.hasOwnProperty(hookIndex)) {
    ...
    const layoutEffectHook = component.hooks[hookIndex];
    if (dependencies !== null) {
      for (let i = 0; i < dependencies.length; i++) {
...
const executeUseAsyncCallback = (hook, callback, newDependencies, activateSuspense) => {
...
export const useAsync = (callback, dependencies, activateSuspense = true) => {
  const [component, hookIndex] = useHook();
  if (!component.hooks.hasOwnProperty(hookIndex)) {
    component.hooks[hookIndex] = {
...
export function Suspense({ children, fallback }) {
  ...
  if (this.loadingComponents.size && fallback)
