	...
	sections: [
		...
		},
		{
			...
			content: (
				<>
					<p>
						Effect hooks are what allow to execute a specific <b>callback</b> when one of your{' '}
						...
						<ul>
							...
							<li>
								...
								The <i>useEffect</i> hook is preferred, because it'll not saturate the JS call
								stack.
							</li>
						</ul>
					</p>
					...
					<p>
						...
						<ul>
							...
							<li>
								...
								function and elaborate its result only when a dependency changes, rather than on
								every render.
							</li>
						</ul>
					</p>
		...
		},
		{
			...
			content: (
				<>
					<p>
						A Context hook will let you <b>obtain a value provided by another parent element</b>,
						more specifically, a <code>Context.Provider</code> element. There is only one context
						hook:
						<ul>
							<li>
								<Link to='useContext'>useContext</Link> - Will return the value provided by the
								closest parent <code>Context.Provider</code> of the specified context. If there is
								not one, the default value of the context will be returned instead. The component
								that uses this hook will automatically re-render whenever the value provided by the
								provider changes.
							</li>
						</ul>
					</p>
					...
					<p>
						...
						<ul>
							...
							<li>
								...
								<b>Expose the state</b>. Commonly used in combination with the <i>useRef</i> hook.
							</li>
							<li>
								...
								<Link to='/docs/components/suspense'>Suspense</Link> component, allowing to easily
								handle the loading state of the component.
							</li>
							<li>
								<Link to='useHook'>useHook</Link> - The useHook is a special hook used to create
								your own advanced hooks. Should only be used if the current hooks are not enough to
								satisfy your needs.
							</li>
						</ul>
					</p>
